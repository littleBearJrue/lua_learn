...
function luaUtil.serialize(t, tabName)
    local function dump(value, desciption, nesting)
	    ...
	    local function _dump(value, desciption, indent, nest, keylen)
	        ...
	        elseif type(value) == "table" and value.className__ == "TableDB" then
            	result[#result +1 ] = table_format("%s%s%s = *isTableDB*", indent, _dump_value(desciption), spc)
	        else
...
--[[--
把一个table存储到本地，可以直接require使用
@string tabName 表名和文件名
@param table 表
@return string
@usage
dumpToFile("Test",{test = 1,a = "2"})
--local test = require("Test")
--dump(test)

]]
function dumpToFile(tabName,table)
	if sys_get_string == nil then
		return "";
	end
	return luaUtil.dumpToFile(tabName,table);
end
